...
| `measurement_tools/fuse_test_voltmeter/` | Pico ADC probe across a battery→fuse→resistor loop; arm switch (GP15) gates trip/reset detection so battery connect/disconnect isn't misread as a trip | bootstrap / concurrent measurement tool | 2026-08-28 — arm switch toggles ARMED/DISARMED correctly, loaded reading steady ~1.36–1.50V across runs (matches SPICE prediction once scaled to this cell's actual ~1.6V open-circuit voltage); fuse rewired directly onto the power rail (extra jumper/power wire removed), baseline re-confirmed after; deliberate-short trip/reset pass/fail check from `quickstart.md` still not passed — an earlier jumper-across-the-resistor attempt never collapsed the reading (seating/contact issue suspected, see `quickstart.md` § "If shorting the resistor doesn't trip anything"), and it hasn't been retried since the rewire |
